...
mkdir -p "$HOME/techwikiplus"

# 6. .env 파일 생성
echo "6. .env 파일 설정 중..."
if [ ! -f "$HOME/techwikiplus/.env" ]; then
    cat > "$HOME/techwikiplus/.env" << 'EOF'
# User Service 이미지 (CI/CD에서 자동 업데이트됨)
USER_SERVICE_IMAGE=<ECR_REGISTRY>/<REPOSITORY>:<TAG>
...
EOF
    chmod 600 "$HOME/techwikiplus/.env"
    echo ".env 파일이 생성되었습니다. 실제 값으로 수정해주세요!"
    echo "편집하려면: nano ~/techwikiplus/.env"
else
...
echo ""
echo "===== 다음 단계 ====="
echo "1. .env 파일을 실제 값으로 수정하세요: nano ~/techwikiplus/.env"

...
sealed class CustomException(val errorCode: String, override val message: String) : RuntimeException(message) {
    ...
    open class NotFoundException(override val message: String) : CustomException(errorCode = "NOT_FOUND", message) {
        data class UserEmailNotFoundException(val email: String) : NotFoundException("User not found with email: $email")
    ...
    sealed class AuthenticationException(override val message: String) : CustomException(errorCode = "AUTHENTICATION_FAILED", message) {
        data class ExpiredEmailVerification(val email: String) : AuthenticationException("Email verification expired for email: $email. Please request a new verification code.")
        data class PendingUserNotFound(val email: String) : AuthenticationException("Pending user not found for email: $email. Please ensure you have registered and requested verification.")
        data class InvalidVerificationCode(val code: String) : AuthenticationException("Invalid verification code: $code. Please check the code and try again.")
        ...
        class EmailNotVerified : AuthenticationException("Email not verified. Please verify your email before logging in.")
    ...
    // Rate Limit 예외
    data class ResendRateLimitExceeded(override val message: String) : CustomException(errorCode = "RATE_LIMIT_EXCEEDED", message)
}

...
class VerificationCodeUnitTest : FunSpec({
    ...
    test("6자리 숫자로 된 유효한 인증 코드를 생성해야 한다") {
        ...
        val registrationCode = RegistrationCode(validCode)

        registrationCode.value shouldBe validCode
    }
    ...
    test("랜덤한 6자리 인증 코드를 생성할 수 있어야 한다") {
        val code1 = RegistrationCode.generate()
        val code2 = RegistrationCode.generate()
    ...
    test("동일한 코드를 가진 객체는 동등해야 한다") {
        val code1 = RegistrationCode("123456")
        val code2 = RegistrationCode("123456")
        val code3 = RegistrationCode("654321")

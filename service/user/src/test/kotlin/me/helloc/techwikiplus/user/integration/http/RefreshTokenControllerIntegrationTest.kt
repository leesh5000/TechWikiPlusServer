...
class RefreshTokenControllerIntegrationTest : ControllerIntegrationTestSupport() {
    ...
    fun `유효한 refresh token으로 토큰 갱신 시 200 응답과 새로운 토큰들을 반환한다`() {
        // given
        val request = RefreshTokenController.RefreshTokenRequest(
    ...
    fun `access token을 refresh token으로 사용 시 401 응답을 반환한다`() {
        // given
        val request = RefreshTokenController.RefreshTokenRequest(
            refreshToken = validAccessToken // access token을 잘못 사용
        )
    ...
    fun `연속해서 refresh token을 사용해도 정상 동작한다`() {
        // given
        val request1 = RefreshTokenController.RefreshTokenRequest(
            refreshToken = validRefreshToken
        )
        ...
        val request2 = RefreshTokenController.RefreshTokenRequest(

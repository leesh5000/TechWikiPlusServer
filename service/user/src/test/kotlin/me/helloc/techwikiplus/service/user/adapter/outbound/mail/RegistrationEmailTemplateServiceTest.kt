...
class RegistrationEmailTemplateServiceTest : FunSpec({
    ...
    test("인증 이메일 제목이 올바르게 생성되어야 한다") {
        ...
        // When
        val emailContent = emailTemplateService.createVerificationEmailContent(registrationCode)
    ...
    test("인증 코드가 눈에 띄게 표시되어야 한다") {
        // Given
        val registrationCode = RegistrationCode("789012")
    ...
    test("반응형 디자인 요소들이 포함되어야 한다") {
        // Given
        val registrationCode = RegistrationCode("123401")
    ...
    test("보안 관련 경고 메시지가 포함되어야 한다") {
        // Given
        val registrationCode = RegistrationCode("345678")
    ...
    test("이메일 템플릿에 스크립트 태그가 포함되지 않아야 한다") {
        // Given
        val registrationCode = RegistrationCode("456789")
